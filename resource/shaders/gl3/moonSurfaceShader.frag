...
vec4 terrainColor(float height) {
	if (height < 0.1) {
		return vec4(0.28, 0.286, 0.269, 0.1);
	}
	else if (height < 0.6) {
		return vec4(0.386, 0.394, 0.391, 0.1);
	}
	else if (height < 0.75) {
		return vec4(0.586, 0.594, 0.581, 0.1);
	}
	else {
		return vec4(0.086, 0.094, 0.041, 0.6);
	}
}

void main() {
	vec3 fragPos_ = vec3(fragPos.x * 0.2, fragPos.y * 2.0, fragPos.z * 0.2);
	float e = e0 * cnoise(fragPos_ * moonScale * 1.0);
	e += e1 * cnoise(fragPos_ * moonScale * 2.0);
	e += e2 * cnoise(fragPos_ * moonScale * 4.0);
	...
	float specVal = 0.0;
	vec4 tcol = terrainColor(eSum);
	outputColor = vec4(tcol.xyz, 1.0);
	if ((tcol.w < 0.51) && (tcol.w > 0.49)) {
		specColor = vec4(outputColor.xyz, 1.0) * 1.8;
		specVal = 8.0 + 2.0 * eSum + cnoise(fragPos * moonScale * 32.0);
	}
	else if (tcol.w < 0.2) {
		specColor = vec4(outputColor.xyz, 1.0) * 0.5;
		specVal = 1.0 + eSum + cnoise(fragPos * moonScale * 8.0);
	}
	else {
		specColor = vec4(outputColor.xyz, 1.0) * 0.5;
		specVal = 8.0 + 3.0 * cnoise(fragPos * moonScale * 32.0);
	}
